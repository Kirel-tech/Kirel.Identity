    ...
    where TClaimUpdateDto : KirelClaimUpdateDto
{
    ...
    private async Task SyncRoleClaims(TKey roleId, ICollection<TClaimUpdateDto> claims)
    {
        ...
        foreach (var claim in roleClaims)
        {
            ...
            if (!claims.Contains(dtoClaim))
            {
                await DeleteClaimFromRole(claim, roleId);
            }
        }
    }
    ...
    public virtual async Task<TRoleDto> CreateRole(TRoleCreateDto createDto)
    {
        ...
        {
            await AddClaimToRole(claim, newRole.Id);
        }
    ...
    public virtual async Task<PaginatedResult<List<TRoleDto>>> GetRolesList(int page, int pageSize, string search, string orderBy, SortDirection orderDirection)
    {
        page = page < 1 ? 0 : page;
        ...
        {
            orderExpression = PredicateBuilder.ToLambda<TRole>(orderBy);
        }
