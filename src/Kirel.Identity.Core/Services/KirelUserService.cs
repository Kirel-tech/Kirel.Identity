    ...
    where TClaimUpdateDto : KirelClaimUpdateDto
{
    ...
    private async Task SyncUserRoles(TKey userId, List<TKey> roles)
    {
        ...
        foreach (var roleName in userRoles)
        {
            ...
            {
                await DeleteUserFromRole(userId, role.Id);
            }
        }
    }
    ...
    private async Task SyncUserClaims(TKey userId, List<TClaimUpdateDto> dtoClaims)
    {
        ...
        {
            if (!claimsFromDto.Any(claimFromDto => claimFromDto.Type == claim.Type && claimFromDto.Value == claim.Value))
            {
    ...
    public virtual async Task<PaginatedResult<List<TUserDto>>> GetUsersList(int page, int pageSize, string search, string orderBy, SortDirection orderDirection)
    {
        page = page < 1 ? 0 : page;
        ...
        {
            orderExpression = PredicateBuilder.ToLambda<TUser>(orderBy);
        }
    ...
    public virtual async Task<TUserDto> CreateUser(TUserCreateDto createDto)
    {
        ...
        {
            var role = await RoleManager.FindByIdAsync(roleId.ToString());
            if (createDto.Roles.Contains(role.Id))
            {
                await AddUserToRole(appUser.Id, roleId);
            }
        }
        ...
        {
            await AddClaimToUser(claim, appUser.Id);
        }
